...
import { downloadCr02ElementAsPdf } from "../../lib/download-cr02-pdf"
...
export function DeathDeclarationForm({ initialData, isReviewFlow, onReviewSubmit, onCancel, mode = "registrar", isReadOnly, pdfFileName }: DeathDeclarationFormProps = {}) {
  ...
  const preFilledKeys = useMemo(() => {
    ...
    return new Set(Object.entries(initialData).filter(([, value]) => value && value.trim() !== "").map(([key]) => key))
  }, [initialData])
  ...
  useEffect(() => {
    ...
    inputs.forEach((element) => {
      if (element.name && preFilledKeys.has(element.name)) {
        element.classList.add("cr2-prefilled")
      }
    })
  }, [preFilledKeys])
  ...
  const handleDownloadPdf = async () => {
    if (!pdfContentRef.current || !pdfFileName) return

    setIsDownloadingPdf(true)
    setError(null)

    try {
      await downloadCr02ElementAsPdf(pdfContentRef.current, pdfFileName)
    } catch {
